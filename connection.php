<?php
session_start();
$servername = "localhost";$username = "root"; $password = ""; $dbname = "sobarmart";  
// $servername = "localhost";$username = "sobarmart_sobarmart"; $password = "bU.EcVT?YRA&"; $dbname = "sobarmart_sobarmart";  
...
if ($cmd == 'MONTH') {
  $fetchQuery = "SELECT SUM(f_flat_rent) AS total_rent, 
                        SUM(f_c_current_bill) AS total_current_bill, 
                        SUM(f_guard_slry) AS total_guard_salary, 
                        SUM(f_c_center_various) AS total_center_various
                 FROM flat_bill 
                 WHERE f_month = '{$month}' AND f_year = '{$year}'";

  $result = mysqli_query($conn, $fetchQuery);
  $row = mysqli_fetch_assoc($result);

  // Net Amount হিসাব করা
  $totalAmount = $row['total_rent'] - ($row['total_current_bill'] + $row['total_guard_salary'] + $row['total_center_various']);

  // payments টেবিল থেকে মোট পরিশোধিত টাকা নিচ্ছে
  $paymentQuery = "SELECT IFNULL(SUM(f_paid_amount), 0) as paid_amount 
                   FROM payments 
                   WHERE f_month = '{$month}' AND f_year = '{$year}'";

  $paymentResult = mysqli_query($conn, $paymentQuery);
  $paymentRow = mysqli_fetch_assoc($paymentResult);
  $totalPaidAmount = $paymentRow['paid_amount']; // মোট পরিশোধিত টাকা

  // Final Total Calculation (Net Amount + Paid Amount)
  $finalTotal = $totalAmount + $totalPaidAmount;

  // শুধুমাত্র Final Total দেখানো হচ্ছে
  echo "<h3 class='text-center'><b> ৳" . number_format((float)$finalTotal, 0) . " </b></h3>";
}
