                  ...
                  <div class="card-body p-4 shadow-lg rounded" style="background-color: #f8f9fa;">
                    ...
                    <p class="card-text">
                      ...
                      <?php
                          ...
                          $sqlTotal = "SELECT SUM(amount) as total_amount FROM projects WHERE main_project_id = ?";
                          $stmtTotal = $conn->prepare($sqlTotal);
                          $stmtTotal->bind_param("i", $data['id']); // Changed from 's' to 'i' and from 'name' to 'id'
                          $stmtTotal->execute();
                          $resultTotal = $stmtTotal->get_result();
                          $totalRow = $resultTotal->fetch_assoc();
                          echo htmlspecialchars(number_format($totalRow['total_amount'] ?? 0, 0)); // Added null coalescing operator
                        ?>
                    </p>
